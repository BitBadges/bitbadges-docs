/**
 * Offline consistency check between the manifest, the PNGs on disk, and the
 * doc pages that embed them. No browser, no network: safe for CI.
 */
import fs from 'node:fs';
import path from 'node:path';

import { ASSETS_DIR, SCREENSHOTS, type Screenshot } from './manifest';
...
export function checkScreenshots(contentDir: string, manifest: Screenshot[] = SCREENSHOTS): CheckResult {
  ...
  for (const shot of manifest) {
    ...
    if (!fs.existsSync(path.join(contentDir, ASSETS_DIR, shot.file))) {
      problems.push(`missing PNG for manifest entry ${shot.file} (run: bun run screenshots)`);
    }
  ...
  for (const rel of walkMarkdown(contentDir)) {
    for (const file of imageRefs(fs.readFileSync(path.join(contentDir, rel), 'utf8'))) {
      if (!byFile.has(file)) problems.push(`${rel} references ${file}, which is not in the manifest`);
    }
  }
  ...
  if (fs.existsSync(assetsDir)) {
    for (const name of fs.readdirSync(assetsDir)) {
      if (name.endsWith('.png') && !byFile.has(name)) problems.push(`orphan PNG not in the manifest: ${ASSETS_DIR}/${name}`);
    }
  }
