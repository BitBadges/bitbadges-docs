...
bb build vault --backing-coin USDC --uri ipfs://Qm... --quiet \
  | bb deploy --browser --msg-stdin --manager bb1abc...

# sign with a key in the chain binary keyring
bb deploy --with-keyring --from mykey --exec --msg-file col.json

# emit a signable payload for ethers, viem, cosmjs, or an HSM
bb deploy --gen-payload --from bb1abc... --msg-file col.json --gas 600000

# dry run first, then confirm the hash
bb deploy col.json --browser --dry-run --manager bb1abc...
bb tx wait $TXHASH --timeout 120
```
...
```bash
bb deploy [input] [path flag] [flags]
```
...
```bash
bb deploy --browser --msg-file collection.json --manager bb1abc...
```

```json
{ "success": true, "path": "browser", "mode": "sign-and-broadcast", "txHash": "F2318CD6...", "chain": "cosmos" }
```
...
With `--sign-only` the output is `{ "success": true, "path": "browser", "mode": "sign-only", "signedTx": "Cu0M...", "chain": "cosmos" }`. The bytes are the `tx_bytes` shape for `/api/v0/broadcast`; POST `{"tx_bytes": "<bytes>", "mode": "BROADCAST_MODE_SYNC"}`. Use it for custodial submitters, retry control, batching, or sign-now-broadcast-later.
...
```bash
bb deploy --browser --message "I authorize XYZ at $(date -u +%FT%TZ)" --expected-address bb1abc...
```

```json
{ "signature": "LIq6ql...", "address": "bb1lmrs9a9ydq6z32chl0kdfc0ww2zmvx95uh7dk3", "publicKey": "AhUwD2...", "chain": "Cosmos" }
```

Personal-sign any message with the browser wallet. Input is `--message <text>`, `--message-file <path>` (`-` for stdin), or a positional (`-` or `@path`). ETH signatures omit `publicKey` because `personal_sign` is recoverable. This replaces the standalone `sign-with-browser` command.
...
```bash
bb build vault --backing-coin USDC --uri ipfs://Qm... --quiet | bb deploy --gen-payload --from bb1abc... --gas 600000
bb deploy --gen-payload --msg-file col.json --from 0x39A1F08F4B6da42bcFB8EBC8c92A1D04978ad0bD --gas 600000
bb deploy --gen-payload --msg-file col.json --from bb1abc... --with-evm-tx --gas 600000
```
...
```json
{
  "chain": "cosmos+evm",
  "chainId": "bitbadges-1",
  "sender": { "address": "bb1lmrs9a9ydq6z32chl0kdfc0ww2zmvx95uh7dk3", "accountNumber": "7", "sequence": "3", "publicKey": "AhUwD2..." },
  ...
  "evmTx": { "to": "0x0000000000000000000000000000000000001001", "data": "0x059dfe13...", "value": "0", "functionName": "createCollection", "chainId": 50024, "gasLimit": "600000" },
...
| --- | --- | --- |
| `signDirect.signBytes` | `--from bb1...`, or a Cosmos pubkey is on chain | Sign for `SIGN_MODE_DIRECT` |
...
| `evmTx` | `--from 0x...`, `--evm-from`, or `--with-evm-tx` | `wallet.sendTransaction({to, data, value, chainId, gasLimit})` |
| `broadcastEndpoint` | always | Where to POST the assembled `TxRaw` |

| Flag | Description |
| --- | --- |
| `--from <address>` | `bb1...` emits `signDirect` + `legacyAmino`; `0x...` emits `evmTx` (plus `signDirect` if a pubkey is on chain) |
| `--evm-from <0x...>` | EVM address for `evmTx` next to a `bb1` sender. Implies `--with-evm-tx`. |
...
For `--from 0x...` the EVM transaction signs itself with the wallet's secp256k1 key; no separate pubkey. `signDirect` for an ETH user needs a pubkey on chain, which one prior transaction sets. Covers every `tokenization.Msg*` type, which is every `bb build` output. Other modules (IBC, gov) are out of scope. This replaces the standalone `gen-tx-payload` command.

### --burner

```bash
bb build subscription --interval monthly --price 10 --denom USDC \
  --recipient bb1pay... --uri ipfs://Qm... --quiet \
  | bb deploy --burner --msg-stdin --manager bb1real... --local --fund faucet
```

```json
{ "success": true, "ephemeralAddress": "bb1abc...", "recoveryPath": "/home/you/.bitbadges/burners/2026-04-15T13-21-43-900Z-bb1abc....json", "txHash": "A6020AAD...", "collectionId": "28" }
```
...
```bash
bb burner list [--network <name>]
bb burner show <selector>                                        # includes the mnemonic
bb burner resume <selector> --msg-file col.json --manager bb1real... [--fund faucet] [--fee 0] [--fee-denom ubadge] [--gas 400000] [--poll-timeout 60]
bb burner sweep <selector> --to bb1real... [--denom ubadge] [--fee 0] [--gas 200000]
bb burner forget <selector> [-y]
```

`<selector>` is an address or a recovery file path.

## tx status and tx wait

```bash
bb tx status 0F46899E29754227DE90F702754CDC74FD39EA37527F2C1E307655C15E910D81
bb tx status 0xabcdef...                       # EVM hash
bb tx wait $TXHASH --timeout 120 --interval 2
bb tx status $TXHASH | jq -r .data.code
```

```json
{ "ok": true, "data": { "via": "cosmos", "hash": "0F46899E...", "height": "10000000", "code": 0, "gasUsed": "156490", "events": [] }, "warnings": [], "error": null }
```
...
| `tx` | `bb deploy --browser`, `bb build ... --browser` | One transaction; collection messages get the full review sidebar | Hash, or signed bytes with `--sign-only` |
...
```bash
bb auth login --browser --address bb1abc... --port 4849
```
